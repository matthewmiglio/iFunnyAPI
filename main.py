    ...
    def post_thread(self):
        def _to_wrap():
            ...
            while self.running_event.is_set():
                if (
                    is_waiting
                    and time_of_next_post_attempt is not None
                    and time.time() > time_of_next_post_attempt
                ):
                    is_waiting = False
    ...
    def get_images_thread(self):
        def _to_wrap():
            ...
            while self.running_event.is_set():
                if (
                    not is_waiting
                    and time_of_next_scrape_attempt is not None
                    and time.time() > time_of_next_scrape_attempt
                ):
                    is_waiting = False
